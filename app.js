...
function ShowAll(){
  for(let i = 0; i < videosList.length; i++) {
    all_videos += generateRowHTML(videosList[i]);
  }  
  document.querySelector('#results').innerHTML = all_videos;
}
...
function Search(){
  ...
  if(input === ''){
    ...
    for(let i = 0; i < videosList.length; i++){
      let videoDescription = videosList[i].description;
      if(videoDescription.indexOf(input) >= 0){
        found_videos += generateRowHTML(videosList[i]);        
      }
      
    }
    if(found_videos.length === 0){
      let error_message = document.querySelector("#results");
      error_message.style.fontWeight = "bold";
      error_message.innerText = "No videos found with the keyword " + input;
      document.getElementById("txt-search").value = '';
      return;
    }
    document.querySelector('#results').innerHTML = found_videos;
    document.getElementById("txt-search").value = '';
  }
}
